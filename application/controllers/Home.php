...
class Home extends CI_Controller
{
    ...
    public function login_page($is_team_login = '0')
    {
        ...
        // $this->load->library("google_login"); //This is old version
        $this->load->library("sheet");
        $data["google_login_button"] = $this->sheet->set_login_button();
    ...
    function google_login_back()
    {
        ...
        // $this->load->library('Google_login');//this old code of google_login
        $this->load->library('sheet');
        $info = $this->sheet->user_details();
        if ( !empty($info) && isset($info->email) && isset($info->name)) {
            if (file_exists(APPPATH . 'core/licence_type.txt'))
                $this->license_check_action();
    ...
    public function facebook_login_back()
    {
        ...
        if (is_array($info) && !empty($info) && isset($info["name"])) {
            ...
            // if its come from affiliate referal link
    ...
    protected function get_addon_data($path = "")
    {
        ...
        $modules_decoded = !empty($addon_data['modules']) ? json_decode($addon_data['modules'],true) : [];
        $addon_data['module_id'] = !empty($modules_decoded) ? array_keys($modules_decoded) : [];
    ...
    protected function delete_addon($addon_controller_name = "", $sql = array())
    {
        ...
        $hostname = $explode[0] ?? '';
        $hostname = trim($hostname, '/');
        if (in_array($hostname, ['localhost', 'chatpion.test'])) {
            echo json_encode(array('status' => '0', 'message' => $this->lang->line('Addon can not be deleted from localhost.')));
            exit();
        }      
    ...
    protected function csrf_token_check($return_bool=false)
    {
...
public function _email_send_function($config_id_prefix="", $message_org="", $to_emails="", $subject="", $attachement='', $fileName='', $user_id='', $test_mail='') 
   {
    ...
    $email_config_details = $this->basic->get_data($table_name, $where2);

    // Check if the configuration details are retrieved
    if (!is_array($email_config_details) || count($email_config_details) == 0) {
        $status = "Oops! Sorry, no configuration is found.";
        return $status;
    }

    $status = "";

    // Handle different email sending methods
    if ($config_type == 'smtp') {
        foreach ($email_config_details as $send_info) {
            $send_email = trim($send_info['email_address']);
            $smtp_host = trim($send_info['smtp_host']);
        ...
        if (is_array($to_emails) && count($to_emails) > 1) {
            ...
            $no_reply = isset($no_reply_arr[1]) ? "do-not-reply@" . $no_reply_arr[1] : $to_emails[0];
        ...
        try {
            if ($this->email->send()) {
                ...
                $status = $test_mail == 1 ? $this->email->print_debugger() : "error in configuration";
            }
        } catch (Exception $e) {
            $status = "error";
        }
    ...
    if ($config_type == 'mandrill') {
        ...
        $result = $this->email_manager->send_mandrill_email($send_email, $send_name, $to_emails, $subject, $message, $api_id, $attachement, $fileName);

        $status = isset($result['error']) && !empty($result['error']) ? $result['error'] : "Submited";
    }
    ...
    if ($config_type == 'sendgrid') {
        ...
        $status = (isset($result['status']) && !empty($result['status']) && $result['status'] == 'success') ? ($test_mail == 1 ? $result['status'] : 'Submited') : $result['status'];
    }
    ...
    if ($config_type == 'mailgun') {
        ...
        $status = $result['status'] != 'error' ? ($test_mail == 1 ? $result['status'] : "Submited") : $result['status'];
    }
    ...
    protected function send_email_to_autoresponder($settings = '', $email = '', $first_name = '', $last_name = '', $type = 'singnup', $user_id = "0", $tags = '')
    {
        ...
        if (isset($settings_array['google_sheet']) && !empty($settings_array['google_sheet'])) {
            ...
            {			
                ...
                foreach ($settings_array['google_sheet'] as $key => $value) {
                    if(empty($value)) continue;
    
                    $explode_value = explode('-',$value);
                    $id = $explode_value[0];
                    $sheet_tab_name = $explode_value[1];
                    

                    $google_sheet_account_data = $this->db
                                                    ->select(['google_accounts.access_token', 'google_accounts.refresh_token', 'google_sheets.sheet_id', 'google_sheets.google_account_id'])
                                                    ->from('google_sheets')
                                                    ->join('google_accounts', 'google_accounts.id = google_sheets.google_account_id', 'left')
                                                    ->where('google_sheets.id', $id)
                                                    ->get()
                                                    ->row_array();
                    if(empty($google_sheet_account_data)) return false;
                    $this->sheet->access_token = $google_sheet_account_data['access_token'];
                    $this->sheet->refresh_token = $google_sheet_account_data['refresh_token'];
                    $sheet_id = $google_sheet_account_data['sheet_id'];
                    $sheet_account_id = $google_sheet_account_data['google_account_id'];
                    $google_sheet_data_append = [
                        !empty($first_name) ? $first_name : 'null',
                        !empty($email) ? $email : 'null',
                    ];
                    //Insert into Log table
                    $result = $this->sheet->append_google_sheet($sheet_id,$sheet_account_id,$sheet_tab_name,$google_sheet_data_append);
                    $result_array = json_decode($result, TRUE);
                    $status = isset($result_array['errors']) ? $result_array['errors'][0]['message'] : "Success";
                    $now_time = date('Y-m-d H:i:s');
                    $insert_data = array('user_id' => $user_id, 'settings_type' => $type, 'status' => $status, 'email' => $email, 'auto_responder_type' => "Email Autoresponder", 'api_name' => 'Google Sheet', 'response' => $result, 'insert_time' => $now_time, 'mailchimp_config_id' => '');
                    $this->basic->insert_data("send_email_to_autoresponder_log", $insert_data);
                ...
                if (isset($processing_data['conditions'])) {
                    ...
                    if ($processing_data['conditions'] != "") {
                        ...
                        if (!empty($conditions)) {
                            ...
                            $condition_status = $this->check_all_condition($rules, $condition_type, $subscriber_info[0], $customer_field_information);
    ...
    protected function insert_livechat_data($insert_data=[],$check_access=true){
        ...
        if($has_livechat_access){
            ...
            $currtime = convert_datetime_to_timezone($currtime,'UTC','','Y-m-d H:i:s',$this->config->item('time_zone'));
            // dd($currtime);
    ...
    protected function http_api_trigger($settings_http_api_id=null,$subscriber_unique_id=null,$subscriber_unique_field='subscribe_id',$settings_http_api_data=null,$sandbox=false){ // accepts both id and subscriber_id, settings_http_api_data used in sandbox
        $this->load->helper('http_api/http_api');
        if((empty($settings_http_api_id) && empty($settings_http_api_data)) || empty($subscriber_unique_id)){
           return $sandbox ? ["data_error"=>$this->lang->line("HTTP API or Subscriber ID not provided.")] : false;
        }

        if (empty($settings_http_api_data)) {
            $this->db->from('settings_http_apis')->where('id', $settings_http_api_id);
            if (!$sandbox) $this->db->where('status', '1');
            $settings_http_api_data = $this->db->get()->row();
        }

        if(empty($settings_http_api_data)) {
            return $sandbox ? ["data_error"=>$this->lang->line("HTTP API connection not found.")] : false;
        }
        if(empty($subscriber_unique_field)) $subscriber_unique_field = "subscribe_id";
        $api_type = $settings_http_api_data->api_type;
        $user_id = $settings_http_api_data->user_id;
        $api_endpoint_url = $settings_http_api_data->api_endpoint_url;
        $api_method = $settings_http_api_data->api_method;
        $mapping_data = !empty($settings_http_api_data->mapping_data) ? json_decode($settings_http_api_data->mapping_data,true) : [];

        $api_data = !empty($settings_http_api_data->api_data) ? json_decode($settings_http_api_data->api_data,true) : [];
        $body_data_type = $api_data["body_data_type"] ?? "raw";
        $header_data = $api_data["header_data"] ?? [];
        $body_data = $api_data["body_data"] ?? [];
        $body_data_json = $api_data["body_data_json"] ?? "";
        $option_data = $api_data["option_data"] ?? [];
        $cookie_data = $api_data["cookie_data"] ?? [];

        $subscriber_table_name = "messenger_bot_subscriber";
        $custom_field_table_name = "user_input_custom_fields";
        $error_log_table_name = "messenger_bot_reply_error_log";
        $custom_field_map_table_name = "user_input_custom_fields_assaign";
        $custom_field_map_unique_field = "subscriber_id";
        $custom_field_map_bot_id_field = "page_id";
        $custom_field_map_custom_field_id_field = "custom_field_id";

        $this->db->where($subscriber_unique_field, $subscriber_unique_id);
        $subscriber_data = $this->db->get($subscriber_table_name)->row();    
        if(empty($subscriber_data)) {
            return $sandbox ? ["data_error"=>$this->lang->line("Subscriber not found.")] : false;
        }
        if($subscriber_data->user_id != $user_id) {
            return $sandbox ? ["data_error"=>$this->lang->line("Subscriber does not belong to the HTTP API.")] : false;
        }
        $bot_id = $subscriber_data->{$custom_field_map_bot_id_field} ?? 0;
        $subscriber_auto_id = $subscriber_data->id ?? 0;
        $subscriber_id = $subscriber_data->subscribe_id ?? "";

        $subscriber_custom_field_data = $this->db->where($custom_field_map_bot_id_field, $bot_id)
            ->where($custom_field_map_unique_field, $subscriber_id)
            ->get($custom_field_map_table_name)->result();
        $subscriber_custom_field_data_mapped = array();
        foreach ($subscriber_custom_field_data as $value) {
            $subscriber_custom_field_data_mapped[$value->{$custom_field_map_custom_field_id_field}] = $value->custom_field_value;
        }
        ...
        foreach($header_data as $value){
            ...
            if($value["type"]=="static"){
                ...
                if(is_numeric($value["value"])){
                    $parsed_value = $subscriber_custom_field_data_mapped[$value["value"]] ?? "";
                } else {
                    $parsed_value =  $subscriber_data->{$value["value"]} ?? "";
                }
            }
            $header_data_mapped[$value["key"]] = $parsed_value;
        }

        // replace system fields and custom fields
        $find = [
        ...
        $full_name = isset($subscriber_data->full_name) && !empty($subscriber_data->full_name) ? $subscriber_data->full_name : $subscriber_data->first_name." ".$subscriber_data->last_name;
        $replace = [
            $subscriber_data->first_name ?? "#LEAD_USER_FIRST_NAME#",
            $subscriber_data->last_name ?? "#LEAD_USER_FIRST_NAME#",
            $full_name ?? "#LEAD_USER_FULL_NAME#",
            $subscriber_data->email ?? "#LEAD_USER_EMAIL#",
            $subscriber_data->phone_number ?? "#LEAD_USER_PHONE#",
            $subscriber_data->gender ?? "#LEAD_USER_GENDER#",
            $subscriber_data->profile_pic ?? "#LEAD_USER_AVATAR#",
            $subscriber_data->client_thread_id ?? "#LEAD_USER_CHAT_ID#",
            $subscriber_data->subscribe_id ?? "#LEAD_USER_SUBSCRIBER_ID#",
        ];
        $custom_field_data = $this->db->select("id, name")->where("user_id", $user_id)->get($custom_field_table_name)->result();
        foreach ($custom_field_data as $value) {
            $find[] = "#" . $value->name . "#";
            $replace[] = $subscriber_custom_field_data_mapped[$value->id] ?? "";
        }

        foreach($find as $fk=>$fv){
            if(isset($replace[$fk])) $api_endpoint_url =  str_replace($fv,urlencode($replace[$fk]),$api_endpoint_url);
        }
        if($body_data_type=='json') {
            $body_data_mapped = $body_data_json;
            $body_data_mapped = str_replace($find,$replace,$body_data_mapped);
        }
        else{
            $body_data_mapped = [];
            foreach($body_data as $value){
                ...
                if($value["type"]=="static"){
                    ...
                    if($body_data_type=="binary") $parsed_value =  $file_url;
                    else $parsed_value =  curl_file_create($file_url);
                }
                $body_data_mapped[$value["key"]] = $parsed_value;
            }
        }

        $option_list = get_curl_constants();
        $option_data_mapped = [];
        foreach($option_data as $value){
            $data_type = $option_list[$value["key"]]["type"] ?? 'string';

            $parsed_value = $value["value"] ?? "";
            if($data_type=='boolean') $parsed_value = (bool) $parsed_value;
            else $parsed_value = (int) $parsed_value;

            $option_data_mapped[$value["key"]] = $parsed_value;
        }
        ...
        foreach($cookie_data as $value){
            ...
            $cookie_data_mapped[$value["key"]] = $parsed_value;
        }

        $last_call_data_mapped = http_api_run_curl($api_endpoint_url,$api_method, $body_data_mapped, $body_data_type, $header_data_mapped, $option_data_mapped, $cookie_data_mapped);
        if($sandbox) return $last_call_data_mapped;
        $data_error = $last_call_data_mapped["data_error"] ?? "";
        $last_call_data_mapped = array_to_obj($last_call_data_mapped);
        $curtime = date("Y-m-d H:i:s");

        $formatter_data = $this->db->where('settings_http_api_id', $settings_http_api_id)->get('settings_http_api_formatters')->result();
        $formatter_data_mapped = [];
        $variables = [];
        foreach ($formatter_data as $key=>$value){
            $formatter_data_mapped[$value->id] = (array) $value;
            $params = json_decode($value->params);
            foreach($params as $k=>$v){
                if($v != '[space]' && str_starts_with($v,'[') && str_ends_with($v,']')){
                    $var = ltrim($v,'[');
                    $var = rtrim($var,']');
                    $var_val = '$last_call_data_mapped->'."{'".str_replace('>',"'}->{'",$var)."'}";
                    $variables[$v] = eval("return isset($var_val) ? $var_val : null;");
                }
            }
        }

        $update_subscriber_data = [];
        $update_subscriber_custom_field_data = [];
        $custom_field_ids = [];
        foreach($mapping_data as $k=>$v){
            $var = '$last_call_data_mapped->'."{'".str_replace('>',"'}->{'",$v["variable"])."'}";
            $field = $v["field"] ?? null;
            $input =  eval("return isset($var) ? $var : null;");
            $formatter = [];
            if(isset($v["formatter"]) && !empty($v["formatter"])){
                foreach($v["formatter"] as $f){
                    $formatter[] = $formatter_data_mapped[$f] ?? [];
                }
            }
            $formatted_input =  workflow_formatter($input,$formatter,$variables);
            if(is_object($formatted_input)) $formatted_input = "";
            ...
            if(is_numeric($field)){
                ...
                $update_subscriber_custom_field_data[] = [
                    ...
                    $custom_field_map_custom_field_id_field => $field,
                    "custom_field_value" => $formatted_input
                ];
            }
            else{
                $update_subscriber_data[$field] = $formatted_input;
            }
        }
        ...
        try {
            ...
            if (!empty($update_subscriber_data)) {
                $this->db->where("id", $subscriber_auto_id)->update($subscriber_table_name, $update_subscriber_data);
            }
        
            if (count($update_subscriber_custom_field_data) > 0 && !empty($custom_field_ids)) {
                $this->db->select("id");
                $this->db->where("user_id", $user_id);
                $this->db->where_in("id", $custom_field_ids);
                $custom_field_data = $this->db->get($custom_field_table_name)->result();
                
                $custom_field_data_found_mapped = [];
                foreach ($custom_field_data as $cf) {
                    $custom_field_data_found_mapped[$cf->id] = true;
                }
        
                foreach ($update_subscriber_custom_field_data as $row) {
                    if (!isset($custom_field_data_found_mapped[$row[$custom_field_map_custom_field_id_field]])) continue;
                    $check_where = $row;
                    unset($check_where["custom_field_value"]);
                    
                    $this->db->select("id");
                    $exist = $this->db->where($check_where)->get($custom_field_map_table_name)->row();
        
                    if (!empty($exist)) {
                        $update_id = $exist->id;
                        $this->db->where("id", $update_id)->update($custom_field_map_table_name, [
                            "custom_field_value" => $row["custom_field_value"] ?? "",
                            "assaign_time" => $curtime
                        ]);
                    } else {
            ...
            $update_settings_http_api_call_data = [
                "subscriber_unique_id" => $subscriber_id,
                "settings_http_api_id" => $settings_http_api_id,
                "api_data" => json_encode([
                    "body_data_type" => $body_data_type,
                    "header_data" => $header_data_mapped,
                    "body_data" => $body_data_mapped,
                    "cookie_data" => $cookie_data_mapped
                ]),
                "api_response" => json_encode($last_call_data_mapped),
            ...
            $this->system_message_insert_into_conversation($subscriber_id, $bot_id, "bot", "HTTP API triggered", $api_type);
        
        } catch (Exception $e) {
            $this->db->trans_rollback(); // Rollback transaction in case of an error
