...
class Visual_flow_builder extends Home
{
    ...
    public function flowbuilder_submit()
    {
        ...
        foreach ($bot_settings_array as $key => $value) {
            ...
            if (is_array($value['google_sheets']) && !empty($value['google_sheets'])) $google_sheets = implode(',', $value['google_sheets']);
            else $google_sheets = '';
            ...
            if(!empty($google_sheets)){
                $insert_data['google_sheet_ids'] = $google_sheets;
                $insert_data_to_bot['google_sheet_ids'] = $google_sheets;
            }
    ...
    public function return_single_reply_array($reply=[], $white_listed_domain_array=[], $page_table_id=0, $media_type='fb', $visual_flow_campaign_id=0)
    {
        foreach ($reply as $single_reply_key => $single_reply_value) 
        {
            ...
            else if ($single_reply_value['reply_type'] == 'httpApiInput') {
                $reply_bot[$single_reply_key]['type'] = 'httpApi';
                $reply_bot[$single_reply_key]['httpApiId'] = $single_reply_value['httpApiId'] ?? '';
            }
            ...
            else if($single_reply_value['reply_type'] == 'userInputFlowInput')
            {
                ...
                $webhook_url = $single_reply_value['webhook_url'] ?? '';
                if (isset($single_reply_value['google_sheets']) && is_array($single_reply_value['google_sheets']) && !empty($single_reply_value['google_sheets'])) $google_sheets = implode(',', $single_reply_value['google_sheets']);
                else $google_sheets = '';
                ...
                if($flow_campaign_id == 'new')
                {
                    ...
                    if (is_array($single_reply_value['user_input_flow_campaign_info']['google_sheets']) && !empty($single_reply_value['user_input_flow_campaign_info']['google_sheets']))
                    $google_sheets = implode(',', $single_reply_value['user_input_flow_campaign_info']['google_sheets']);
                    else $google_sheets = '';
                    ...
                    if(!empty($google_sheets)) $insert_update_data['google_sheet_ids'] = $google_sheets;
                ...
                {
                    if (!empty($google_sheets))
                        $this->basic->update_data('user_input_flow_campaign',['id' => $flow_campaign_id],['google_sheet_ids' => $google_sheets]);
    ...
    public function language_file() {
        ...
        ?>
        "use strict";
        var button_html_google_sheet_add = '<?php echo $this->lang->line("Send data to Google Sheets"); ?>';
        ...
        var http_api_name = '<?php echo $this->lang->line("HTTP API"); ?>';
        var http_api_socket_input_reply = '<?php echo $this->lang->line("Message"); ?>';
        var http_api_socket_output_next = '<?php echo $this->lang->line("Compose Next Message"); ?>';
        var http_api_modal_title = '<?php echo $this->lang->line("Configure HTTP API"); ?>';
        var http_api_list = '<?php echo $this->lang->line("HTTP API List"); ?>';
        var http_api_create = '<?php echo $this->lang->line("Create"); ?>';
        var http_api_edit = '<?php echo $this->lang->line("Edit"); ?>';
        var http_api_report = '<?php echo $this->lang->line("Report"); ?>';
        var http_api_refresh = '<?php echo $this->lang->line("Refresh"); ?>';
        var http_api_modal_error_msg = '<?php echo $this->lang->line("Please select a HTTP API template"); ?>';
        ...
        var audio_html_info_supported_media_ig = '<?php echo $this->lang->line("Supported types: acc, m4a, wav, mp4"); ?>';
        ...
        var video_html_info_supported_media_ig = '<?php echo $this->lang->line("Supported types: mp4, ogg, avi, mov, webm"); ?>';
    ...
    public function get_google_sheets()
    {
        ...
        $select = ['google_sheets.id', 'google_sheets.name', 'email', 'sheet_names'];
        $google_sheets_data = $this->basic->get_data('google_sheets',['where'=>['google_sheets.user_id'=>$this->user_id]],$select,$join,$limit = '', $start = '', $order_by = 'google_sheets.name ASC');

        $grouped_sheets = [];
        foreach ($google_sheets_data as $sheet) {
            $grouped_sheets[$sheet['email']][] = $sheet;
        }

        // Start building the select dropdown
        $google_sheets = '';

        // Loop through each group (grouped by email)
        foreach ($grouped_sheets as $email => $sheets) {
            $google_sheets .= "<optgroup label='{$email}'>"; // Create the optgroup with the email as the label

            // Loop through each sheet within the current email group
            foreach ($sheets as $sheet) {
                $search_key = $sheet['id'];    // Use the id as the option value
                $search_name = $sheet['name']; // Use the name as the option label
                $sheet_names = $sheet['sheet_names'];
                $sheet_names_array = explode(',', $sheet_names);
                foreach ($sheet_names_array as $single_sheet_name)
                    // Add the option tag
                    $google_sheets .= "<option value='{$search_key}-{$single_sheet_name}'>{$search_name} [{$single_sheet_name}]</option>";
            }

            $google_sheets .= "</optgroup>"; // Close the optgroup
        }

        // Return the response in JSON format
        $response['googleSheets'] = $google_sheets;
        echo json_encode($response);
    }

    public function get_http_api_dropdown($channel = '')
    {
        $str = '<option value="">' . $this->lang->line('Select HTTP API') . '</option>'; // Adjusted for CodeIgniter language line
        ...
        if (!empty($channel)) {
            ...
            foreach ($info_type as $value) {
                ...
                $name = $value->api_name;
                if ($name != "") $name .= " : ";
                $str .= "<option value='{$id}'>" . $name . $value->api_method . "</option>";
            }
        }

        echo json_encode(array('dropdown_str' => $str, 'status' => '1'));
    }
}